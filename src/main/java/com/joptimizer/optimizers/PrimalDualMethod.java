...
public class PrimalDualMethod extends OptimizationRequestHandler {
    ...
    public int optimize() throws Exception {
        ...
        if(X0==null){
            ...
            if(X0NF!=null){
                ...
                if(maxValue<0 && rPriX0NFNorm<=getToleranceFeas()){
                    if (debug)
                    {
                        log.debug("the provided not-feasible starting point is already feasible");
                    }
                    X0 = X0NF;
                }
            }
        ...
        if(maxValue >= 0){
            throw new Exception("Initial point must be strictly feasible. Found maxValue: " + maxValue + " at maxIndex: " + maxIndex);
        }

        final double rPriX0Norm = Math.sqrt(ALG.norm2(rPri(X0)));
        if(rPriX0Norm > getToleranceFeas()){
            log.error("rPriX0Norm  : " + rPriX0Norm);
            log.error("ineqX0      : " + ArrayUtils.toString(fiX0.toArray()));
